      ...
      <div className="max-w-[1400px] mx-auto w-full h-20 flex items-center justify-between">
        <div className="flex items-center gap-3 lg:gap-4 xl:gap-8 min-w-0">
          <Link to="/" className="flex items-center gap-2.5 shrink-0 group">
            ...
            <div className="flex flex-col items-start leading-none gap-0.5 select-none text-left">
              <span className="font-serif text-sm sm:text-base font-black tracking-wider text-slate-100 uppercase">Prince</span>
              <span className="font-serif text-[11px] sm:text-xs font-black italic tracking-widest text-brand-blue uppercase">Star</span>
            </div>
          </Link>

          {/* Nav Links */}
          {location.pathname !== '/chat' && (
            <div className="hidden lg:flex items-center gap-1 xl:gap-2.5 py-2 shrink-0 min-w-0">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={cn(
                    "px-1.5 xl:px-3 py-1 text-[9px] xl:text-xs font-black uppercase tracking-widest transition-all whitespace-nowrap border-b-2 pb-1.5",
              ...
              {profile?.badges?.length > 0 && (
                  <div className="hidden lg:flex items-center gap-1 xl:gap-2 pl-2 xl:pl-4 border-l border-white/5 h-10 shrink-0 ml-1 xl:ml-2">
                    <button
                      onClick={() => setIsBadgeModalOpen(true)}
                      className="flex items-center gap-1 xl:gap-2 group transition-all"
                    >
                      <div className="w-8 h-8 rounded-lg overflow-hidden shadow-lg shadow-blue-500/20 border border-white/10 bg-[#0b0f1a] transition-transform group-hover:scale-105 active:scale-95 shrink-0">
                        <img src={`${import.meta.env.BASE_URL}badge-logo.png`} alt="Badges" className="w-full h-full object-cover" />
                      </div>
                      <div className="flex flex-col items-start leading-none text-left">
                        <div className="flex items-center gap-1">
                          <span className="text-xs xl:text-sm font-bold text-white tracking-tight">{(profile.totalBadges || 0) + (profile.badges?.length || 0)}</span>
                          <div className="w-3 h-3 rounded-sm overflow-hidden opacity-60 shrink-0">
                            <img src={`${import.meta.env.BASE_URL}badge-logo.png`} alt="" className="w-full h-full object-cover" />
                          </div>
                        </div>
                        <span className="text-[7px] font-black uppercase tracking-widest text-slate-500 mt-0.5 group-hover:text-blue-400 transition-colors whitespace-nowrap">View History</span>
                      </div>
                    </button>
                  </div>
              )}
        ...
        <div className="flex lg:hidden items-center gap-2 ml-4">
          <button
            ...
            className="p-2 sm:p-2.5 bg-white/5 hover:bg-white/10 rounded-xl border border-white/10 text-white transition-all active:scale-95"
            aria-label="Toggle Menu"
          >
            {isOpen ? <X className="w-5 h-5 sm:w-6 h-6" /> : <Menu className="w-5 h-5 sm:w-6 h-6" />}
          </button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="lg:hidden absolute top-20 left-0 right-0 bg-[#070a13]/98 backdrop-blur-2xl border-b border-white/10 overflow-y-auto max-h-[calc(100vh-5rem)] z-40 shadow-2xl flex flex-col px-6 py-6 pb-8 space-y-4"
          >
              ...
              <div className="pt-4 border-t border-white/5">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg overflow-hidden border border-white/10 bg-[#0b0f1a]">
                      <img src={`${import.meta.env.BASE_URL}badge-logo.png`} alt="Badges" className="w-full h-full object-cover" />
                    </div>
                    <span className="text-xs font-bold text-white uppercase tracking-wider">History & Badges</span>
                  </div>
                  <span className="text-xs font-bold text-brand-blue">
                    {(profile.totalBadges || 0) + (profile.badges?.length || 0)} Badges
                  </span>
                </button>
              </div>
            )}
